...
import { Calendar, Upload, Plus, Image as ImageIcon, Video, Loader2 } from "lucide-react"
...
export const AddEventDialog = ({ onEventAdd, congregations = [] }: AddEventDialogProps) => {
  const { user } = useAuth()
  const [open, setOpen] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [mediaPreview, setMediaPreview] = useState<string>("")
  const [isMultiDay, setIsMultiDay] = useState(false)
  const [internalCongregations, setInternalCongregations] = useState<any[]>([])
  
  const [formData, setFormData] = useState<EventFormData>({
    congregationId: user?.congregationId || 0,
    title: "",
    description: "",
    day1Date: "",
    day1StartTime: "",
  ...
  useEffect(() => {
    if (open && user && (user.role === 'admin_geral' || user.hasGeneralAccess || (user.role === 'admin_sede' && user.congregation?.isHeadquarters))) {
      fetchCongregations()
    }
  }, [open, user])

  useEffect(() => {
    if (user?.congregationId && formData.congregationId === 0) {
      setFormData(prev => ({ ...prev, congregationId: user.congregationId! }))
    }
  }, [user])
  ...
  const activeCongregations = congregations.length > 0 ? congregations : internalCongregations
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare payload with new API fields
      const payload: any = {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-white shadow-lg shadow-amber-500/25">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {!isMultiDay ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="day1Date" className="text-base font-semibold">
                  Data *
                </Label>
                <Input
                  id="day1Date"
              ...
              <div className="space-y-2">
                <Label htmlFor="day1StartTime" className="text-base font-semibold">
                  Horário Início *
                </Label>
                <Input
                  id="day1StartTime"
              ...
              <div className="space-y-2">
                <Label htmlFor="day1EndTime" className="text-base font-semibold">
                  Horário Término *
                </Label>
                <Input
                  id="day1EndTime"
